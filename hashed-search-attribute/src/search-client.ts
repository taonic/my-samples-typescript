...
function printUsage() {
  ...
  console.log(
    '  npx ts-node src/search-client.ts --query \'HashedEmail = "<hash>" AND PaymentAmount >= 1000\''
  );
  process.exit(1);
}

function parseArgs(): { flag: string; value: string } {
...
function buildQuery(flag: string, value: string): string {
  switch (flag) {
    case '--email': {
