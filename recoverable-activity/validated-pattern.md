...
- **Product tags:** Temporal Workflows, Temporal Activities, Temporal Signals, Temporal Queries, Temporal Search Attributes, Temporal TypeScript SDK v1.13, Temporal Visibility
- **Authors:** Tao Guo
...
![A blocked Workflow showing the failure details and a form to send a corrective Signal](assets/cover-screen.png)
...
In this implementation plan, you will build and verify the recoverable data validation pipeline.
You will begin by defining the data models that flow through Workflows and Activities.
Next, you will create the validation Activities that throw non-retryable errors on invalid data or business rule violations.
You will then implement the Workflow with the `recoverableStep` pattern that pauses on failure and resumes on Signal.
You will expose the pipeline through a web service and write tests to validate your orchestration logic locally.
...
export async function verifyIncome(
  ...
  // Simulate unrecognized employer — cannot verify, requires manual correction
...
export async function runCreditCheck(
  ...
  // Simulate a malformed SSN — no retry will fix this
...
export async function orderAppraisal(
  ...
  // Simulate an invalid property address
...
export async function performTitleSearch(
  ...
  // Simulate an invalid property ID
...
export async function homeLoanWorkflow(application: LoanApplication): Promise<LoanState> {
  ...
  // Each step is wrapped in recoverableStep so a failure pauses — not crashes — the pipeline.
...
async function run() {
  // Connect to the Temporal Cluster — all client operations go through this connection
  ...
  // List all loan workflows using the Visibility API (List Filter query syntax)
  app.get('/api/workflows', async (_req, res) => {
    try {
      const workflows: any[] = [];
      // client.workflow.list() returns an async iterator over matching Workflow Executions
  ...
  // Filter Workflows by Search Attributes — enables routing to the right resolution resource
  app.get('/api/workflows/search', async (req, res) => {
    try {
      const { failedActivity, status } = req.query;
      // Build a List Filter dynamically from query params
  ...
  // Start a new loan Workflow Execution
  ...
  // Send a corrective Signal to a blocked Workflow
...
The web service exposes four endpoints.
The `GET /api/workflows` endpoint lists all active Workflows by querying the Temporal Visibility API.
The `GET /api/workflows/search` endpoint filters Workflows by `FailedActivity` and `LoanStatus` Search Attributes, enabling operators to find which applications need a specific type of correction.
The `POST /api/workflows` endpoint starts a new loan processing Workflow.
The `POST /api/workflows/:workflowId/fix` endpoint sends a corrective Signal to a specific Workflow.

The search endpoint demonstrates how custom Search Attributes enable routing.
An operations team can query for all Workflows blocked on income verification and assign them to the HR data team.
An automated agent can query for all Workflows blocked on credit checks and apply corrections from an upstream data source.
The same query interface supports both human and automated resolution paths.
...
The Worker connects to the Temporal Cluster and registers both the Workflow and Activity implementations.
The `workflowsPath` property points to the compiled Workflow module, and the `activities` property provides the Activity function implementations.
...
![Dashboard showing 3 blocked Workflows and 1 completed](assets/list-view.png)
...
![Sending a corrective Signal to fix an unknown employer](assets/fix-employer-sample.png)
...
![Workflow completed after fixing the employer name](assets/successful-execution-after-fixing-employer.png)
...
![Event History in the Temporal Web UI showing the Signal and Activity completions](assets/event-history-on-temporal.png)
...
After sending the Signal, the Workflow wakes up, patches the field on the application object, and retries the failed Activity with the corrected value.
If the Activity succeeds, the pipeline advances to the next step.
The Workflow records the fix in the `fixHistory` array, providing a complete audit trail.
...
This state is queryable at any point during execution.
After sending corrections and advancing through the pipeline, the Query reflects the updated `completedActivities`, the accumulated `fixHistory`, and the patched `application` data.
This provides full visibility into the lifecycle of every individual Workflow execution.
